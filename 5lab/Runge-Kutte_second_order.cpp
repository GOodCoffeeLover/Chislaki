...
using namespace std;
//метод Рунге-Кутта для решения оду первого порядка y'' = f(x,y, y'), на отрезке [l, r] начальными условиями y(l) = y0, y'(l)=z0
vector<pair<double, double>> Rugne_Kutt(auto f, double l, double r, double y0, double z0, int n){
  ...
  vector<double> z(n+1, 0); //вспомонательный вектор содержащий z[i] = y'_i
  ...
  cout<<"---+"<<setw(width)<<line<<'+'<<setw(width)<<line<<'+'<<setw(width)<<line<<'+'<<setw(width)<<line<<'+'<<setw(width)<<line<<'+'<<setw(width)<<line<<'+'<<setw(width)<<line<<'+'<<setw(width)<<line<<'+'<<setw(width)<<line<<'+'<<setw(width)<<line<<'+'<<setw(width)<<line<<'+'<<endl;
  //вычисление следующей точки и печать её в таблицу  
  for(int i=0; i<n; ++i){
...
//уточнение по формуле Рунге-Ромберна
...
int main(){
  ...
  double yn;
  //решение уравнения при разных шагах
